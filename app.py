...
PROFESSORES = sorted([
    "Prof. Ana Maria de Lima",
    "Prof. Alliny Rodrigues Peixe",
    "Prof. Bibiana Maria Bispo",
    "Danielly Furlaneto Firmino - Assist. Adm.",
    "Prof. Evanilda de Souza Trindade",
    "Prof. Fabiola de Oliveira Herbella",
    "Prof. Felipe Paulo de Souza",
    "Prof. Iwlly Rafaela Mendes",
    "Izaira Moreira Veiga - Pedagoga",
    "Prof. Jaqueline Bonifácio Michelato",
    "Janaína Letícia Panaggio - Secretária",
    "Juliana Aparecida Gonçalves - Pedagoga",
    "Prof. Juliana Ferri",
    "Prof. Katy Tondelli",
    "Prof. Kelen Cristina Leão",
    "Prof. Lorena Cristina de Oliveira Fernandes",
    "Lucas da Costa Ferreira - Assist. Adm.",
    "Prof. Luzia Nogueira",
    "Prof. Marcel Dancini Rodrigues",
    "Prof. Marco Aurélio Sant Ana",
    "Prof. Miriam Aparecida de Souza Dias",
    "Prof. Patrícia Janoni",
    "Romilson Lopes Leite - Inspetor",
    "Thiago José da Rocha - Diretor",
])
...
DISCIPLINAS = [
    ...
    "ELETIVA I",
    "ESTUDO ORIENTADO I",
    "LEITURA REC APREND LINGUA PORT",
    "EMPREENDEDORISMO I",
    "LITER ART E MOVIMENTO I",
    "PENS LOG E CIDAD DIGITAL I",
    "PRATICAS EXPERIMENTAIS I",
    "PROJETO DE VIDA I",
    "REC APREND MATEMATICA",
    "REDACAO E LEITURA I",
...
ALUNOS = [
    {
        "id": 1,
        "nome": "AGATHA VITÓRIA NASCIMENTO",
        "turma": "6º",
        "responsavel": "RAQUEL APARECIDA DE CAMPOS"
    },
]
...
ENCAMINHAMENTOS = [
    "Orientação verbal ao estudante",
    "Conversa individual com o estudante",
    "Mediação pedagógica em sala",
...
def gerar_pdf(dados):
    ...
    filename = f"ocorrencia_{nome_seguro}_{data_arquivo}.pdf"
    ...
    titulo = ParagraphStyle(
        ...
        parent=styles["Title"],
        fontSize=15,
        textColor=colors.HexColor("#08345f"),
        alignment=1,
    )
    ...
    subtitulo = ParagraphStyle(
        ...
        fontSize=10,
        alignment=1,
        textColor=colors.HexColor("#0b5b78"),
    )
    ...
    elementos.append(
        Paragraph(
            "Escola Estadual Padre Manuel da Nóbrega - E.F. Tempo Integral",
            subtitulo
        )
    )
    ...
    elementos.append(Paragraph("Texto da ocorrência", secao))
    ...
    elementos.append(
        Paragraph(
            "Servidor(a) que realizou o registro",
            subtitulo
        )
    )
...
def enviar_email(dados, pdf_path):
    ...
    try:
        ...
        return False, (
            f"PDF gerado, mas a API Brevo retornou erro "
            f"{response.status_code}: {response.text}"
        )

    except Exception as e:
        return False, (
            f"PDF gerado, mas houve erro ao enviar e-mail "
            f"pela API Brevo: {e}"
        )
...
def index():
    ...
    if request.method == "POST":
        ...
        if not aluno:
            flash(
                "Aluno não encontrado. Verifique a turma e o estudante selecionado.",
                "erro"
            )
...
def historico():
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        rows = conn.execute(
            "SELECT * FROM ocorrencias ORDER BY id DESC LIMIT 100"
        ).fetchall()
